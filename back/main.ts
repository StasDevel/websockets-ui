...
import { ServerData, WebSocketWithGameInfo } from './types.js'

const server = new HandmadeESocket()

server.on('reg', (passedData: string, serverData: any, ws: any, wss: any) => {
...
server.on(
  ...
  (
    _: any,
    serverData: any,
    ws: { userGameInfo: { roomInfo: { createdRoomId: any } } },
    wss: any,
  ) => {
...
server.on(
  ...
  (
    passedData: string,
    serverData: { rooms: any },
    ws: {
      userGameInfo: {
        userId: any
        roomInfo: { createdRoomId?: any; roomId?: any }
      }
    },
    wss: any,
  ) => {
...
server.on(
  ...
  (
    passedData: string,
    serverData: { rooms: { [x: string]: { games: any } } },
    ws: { userGameInfo: { roomInfo: { roomId: any } } },
    wss: any,
  ) => {
